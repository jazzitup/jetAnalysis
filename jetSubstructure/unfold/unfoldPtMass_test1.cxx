...
// This file was made to do Martin's comment for the test the unfolding stability.
// Responce matrix is made from reweighting MC 
// input Data specturm is replaced by UN-reweighted MC
...
RooUnfoldResponse* getResponse( int kSample = kPP, int icent = 0, int optX=1, int optY=1,  TH2D* hTruth=0, TH2D* hReco=0, double radius =0.4, bool doReweight=true);
void getDataSpectra( int kSample = kPP, int icent=0,  int optX=1, int optY=1, TH2D* hReco=0, double radius=0.4);


void unfoldPtMass_test1(int kSample = kPP, int optX =1, int optY = 2, double radius= 0.4, int nIter=4) {
  TH1::SetDefaultSumw2();
  int nXbins;
  double xBin[30];
  ...
  RooUnfoldResponse* resUnr[7];
  
  TH2D* hTruth[7];
  TH2D* hReco[7];
  TH2D* hResultMc[7];

  TH2D* hTruthUnr[7]; // Unr = Unreweighted
  TH2D* hRecoUnr[7];
  TH2D* hResultMcUnr[7];
  ...
  for ( int i=0 ; i<=6; i++) {
    ...
    res[i] = getResponse(kSample, i, optX, optY, hTruth[i], hReco[i], radius,true); // Reweight for matrix
    
    c01->cd(1);   hTruth[i]->Draw("colz");
    c01->cd(2);   hReco[i]->Draw("colz");
    c01->SaveAs(Form("pdfs_test1/correlation_coll%d_cent%d_radius%.1f.pdf",kSample,i,(float)radius));
    
    //    getDataSpectra( kSample, icent, optX, optY, hRecoData[i], radius) ;
    hRecoData[i] = (TH2D*)hRecoTemp->Clone(Form("hRecoData_icent%d",icent));
    hTruthUnr[i] = (TH2D*)hTruthTemp->Clone(Form("%s_Unricent%d",hTruthTemp->GetName(),i));

    resUnr[i] = getResponse(kSample, i, optX, optY, hTruthUnr[i], hRecoData[i], radius, false); // No reweight for input spectra
  ...
  for ( int icent=0 ; icent<=6; icent++) {
    ...
    c1a->SaveAs(Form("pdfs_test1/massClosure1_coll%d_icent%d_optX%d_optY%d_radius%.1f_applyMDJ%d.pdf",kSample,icent,optX,optY,(float)radius,(int)applyMDJ));
    c1b->SaveAs(Form("pdfs_test1/massClosure2_coll%d_icent%d_optX%d_optY%d_radius%.1f_applyMDJ%d.pdf",kSample,icent,optX,optY,(float)radius,(int)applyMDJ));
    ...
    c1c->SaveAs(Form("pdfs_test1/xBinClosure1_coll%d_icent%d_optX%d_optY%d_radius%.1f_nIter%d_applyMDJ%d.pdf",kSample,icent,optX,optY,(float)radius,nIter,(int)applyMDJ));
    ////////// end of pT closure 
    ...
    c2a->SaveAs(Form("pdfs_test1/c2a_coll%d_icent%d_optX%d_optY%d_radius%.1f_nIter%d_applyMDJ%d.pdf",kSample,icent,optX,optY,(float)radius,nIter,(int)applyMDJ));     
    c2b->SaveAs(Form("pdfs_test1/c2b_coll%d_icent%d_optX%d_optY%d_radius%.1f_nIter%d_applyMDJ%d.pdf",kSample,icent,optX,optY,(float)radius,nIter,(int)applyMDJ));
    ...
    c2c->SaveAs(Form("pdfs_test1/c2c_coll%d_icent%d_optX%d_optY%d_radius%.1f_nIter%d_applyMDJ%d.pdf",kSample,icent,optX,optY,(float)radius,nIter,(int)applyMDJ));
    ////////// end of pT unfolding
    ...
    for ( int ix = 1 ; ix<= nXbins ; ix++) {
      ...
      if ( optY==8)   hmassUnfSqrt[ix]->SetXTitle("m^{2}/p_{T}^{2}");
      ...
      else if ( optY==8)   hmassUnfSqrt[ix]->SetAxisRange(0.00001,15,"Y");
      
      if ( optY==1)   hmassUnfSqrt[ix]->SetAxisRange(-20,100,"X");
      if ( optY==2)   hmassUnfSqrt[ix]->SetAxisRange(-.05,0.25,"X");
      if ( optY==8)   hmassUnfSqrt[ix]->SetAxisRange(-.05,0.25,"X");
    ...
    c3->SaveAs(Form("pdfs_test1/c3_coll%d_icent%d_optX%d_optY%d_radius%.1f_nIter%d_applyMDJ%d.pdf",kSample,icent,optX,optY,(float)radius,nIter,(int)applyMDJ));
  ...
  if ( selectedCent(periBin) && (kSample == kPbPb ) ){
	...
	jumSun(0,1,200,1);
      ...
      c4->SaveAs(Form("pdfs_test1/RCP_icent%d_optX%d_optY%d_radius%.1f_nIter%d_applyMDJ%d.pdf",icent,optX,optY,(float)radius,nIter,(int)applyMDJ));
	...
	drawBin(xBin,ix,"GeV",0.35,0.8,1,18);
      ...
      c5->SaveAs(Form("pdfs_test1/Cent-Peri_icent%d_optX%d_optY%d_radius%.1f_nIter%d_applyMDJ%d.pdf",icent,optX,optY,(float)radius,nIter,(int)applyMDJ));
      
    }
  }
  
  TFile* fout = new TFile(Form("spectraFiles_test1/unfoldingResult_coll%d_optX%d_optY%d_radius%.1f_nIter%d.root",kSample,optX,optY,(float)radius,nIter),"recreate");
  for ( int i=0 ; i<=6; i++) {
    int icent = i;
    if ( !selectedCent(icent))      continue;
...
//bool doReweight = true;  // MUST BE true for the purpose of this test.
RooUnfoldResponse* getResponse(int kSample,  int icent,  int optX, int optY, TH2D* hTruth, TH2D* hReco, double radius, bool doReweight)
{
  ...
  if ( doJES) {
	...
	jumSun(1,0,1,100);
      ...
      cJes->SaveAs(Form("pdfs_test1/jesDist_kSample%d_icent%d_optX%d_optY%d_ix%d_applyMDJ%d.pdf",kSample, icent, optX, optY,ix,(int)applyMDJ));
	...
	hJER->SetBinContent(ix,iy, hdistMass[ix][iy]->GetRMS() );
      ...
      cJms->SaveAs(Form("pdfs_test1/jmsDist_kSample%d_icent%d_optX%d_optY%d_ix%d_applyMDJ%d.pdf",kSample, icent, optX, optY,ix,(int)applyMDJ));
    ...
    cJes2->SaveAs(Form("pdfs_test1/JESJER_kSample%d_icent%d_optX%d_optY%d_applyMDJ%d.pdf",kSample, icent, optX, optY,(int)applyMDJ));
    //    for ( int ix = 1 ; ix<=nXbins ;ix++) {
    //      for ( int iy = 1 ; iy<=nYbins ;iy++) {
    //        delete hdist[ix][iy];
    //      }
    ...
    for ( int ix=1 ; ix<=nXbins;ix++) {
      ...
      cResp->SaveAs(Form("pdfs_test1/massResponse_kSample%d_icent%d_optX%d_optY%d_ix%d_%d.pdf",kSample, icent, optX, optY,ix,(int)applyMDJ));
  ...
  if (doCheckSmearing) {
    ...
    leg3->Draw();
    cSmear->SaveAs("pdfs_test1/cSmear.pdf");
  }
...
#ifndef __CINT__
int main () { unfoldPtMass_test1(); return 0; }  // Main program when run stand-alone
#endif
