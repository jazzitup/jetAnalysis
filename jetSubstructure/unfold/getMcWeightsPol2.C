...
void getMcWeightsPol2(int kSample = kPP, int icent=0, float weightCut = 10, int nSys=-1) {   // opt1 : mass,   opt2 : m/pT  
  int opt = 772;
  TH1::SetDefaultSumw2();
  ...
  TFile * fout;
  if ( nSys < 0 ) 
    fout = new TFile(Form("reweightFactors/reweightingFactor_weightCut%d_opt%d_flucCut%.1f_factorized_v50_Pol2fit.root",(int)weightCut,opt,(float)flucCut),"update");
  else
