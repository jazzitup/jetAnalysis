...
void getTrkR_noUESubt(int kSample = kPP, int im=1, int icent=0) {    // opt1 : mass,   opt2 : m/pT  
  TH1::SetDefaultSumw2();
  ...
  c2->SaveAs(Form("pdfsJMS/JMS_kSample%d_cent%d_im%d_noUESubt.pdf",kSample,icent,im));
  ...
  f2->SetLineStyle(2);
  //  f2->Draw("same");
  c3->SaveAs(Form("pdfsJMS/trkR_kSample%d_cent%d_massBin%d_noUESubt.pdf",kSample,icent,im));

  TFile* fout = new TFile(Form("R_trk/Rtrk_kSample%d_cent%d_noUESubt.root",kSample,icent),"update");
...
void getMCR(int kSample, int icent, int im, TH2D* hmc) { 
  ...
  if ( kSample == kPbPb ) {
    ...
    jz2 = "jetSubstructure_v52pp_massNoSubt_HION9_jz2_may11.root";
    jz3 = "jetSubstructure_v52pp_massNoSubt_HION9_jz3_may11.root";
    jz4 = "jetSubstructure_v52pp_massNoSubt_HION9_jz4_may11.root";
  }
  ...
  for ( int ijz =2 ; ijz<=4 ; ijz++) {
    ...
    for (Int_t i= 0; i<tr->GetEntries() ; i++) {
      ...
      myJetMc.recoMass = myJetMc.recoMass * 1000; // MeV -> GeV
...
void getDATAR(int kSample, int icent, int im,  TH2D* hdata) {
  ...
  if ( kSample == kPbPb ) {
    ...
    fname = "jetSubstructure_data_HION9_v52_pp_massNoSubt_r4_may11.root";
  }
  ...
  for (Int_t i= 0; i<tr->GetEntries() ; i++) {
    ...
    myJet.recoMass = myJet.recoMass * 1000; // MeV -> GeV
