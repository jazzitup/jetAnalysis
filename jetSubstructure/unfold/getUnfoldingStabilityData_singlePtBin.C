...
void getUnfoldingStabilityData_singlePtBin(int kSample= kPP, int icent = 0, int nIter=1, int optX=1, int optY=2) {
  ...
  for ( int ipt = lowPtBin ; ipt<= highPtBin ; ipt++)  {
    ...
    hdataUnfSq[ipt]->SetTitleSize(20,"Y");
    ...
    gPad->SetLogy();
      ...
      drawBin(xBin,ipt,"GeV",0.2,0.78,1,22);
    if ( ipt == lowPtBin ) {
      drawCentrality(kSample, icent, 0.2,0.86,1,22);
      drawText(Form("Iteration: %d",nIter), 0.7,0.86,4,24);
      TLegend *leg1 = new  TLegend(0.2006099,0.1195233,0.7575544,0.3318368,NULL,"brNDC");
    ...
    hdataRatioSq[ipt]->SetAxisRange(0.,2.1,"Y");
    ...
    hdataRatioSq[ipt]->SetNdivisions(505,"Y");
    hdataRatioSq[ipt]->SetNdivisions(505,"X");
    hdataRatioSq[ipt]->SetTitleSize(.12,"X");
    hdataRatioSq[ipt]->SetTitleSize(0.1,"Y");
    hdataRatioSq[ipt]->SetTitleOffset(0.4,"X");
    hdataRatioSq[ipt]->SetTitleOffset(0.5,"Y");
    hdataRatioSq[ipt]->SetTitleSize(.820,"Y");
    hdataRatioSq[ipt]->SetTitleSize(20,"X");
    hdataRatioSq[ipt]->SetLabelSize(20,"X");
    hdataRatioSq[ipt]->SetLabelSize(20,"Y");
    fixedFontHist(hdataRatioSq[ipt],2,2,20);
