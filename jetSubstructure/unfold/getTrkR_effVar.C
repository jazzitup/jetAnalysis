...
void getTrkR_effVar(int kSample = kPbPb, int icent=0, int ptCut =1 ) {   // opt1 : mass,   opt2 : m/pT  
  TH1::SetDefaultSumw2();
  ...
  int nXbins = 5;
  double xBin[30] = { 150,170,200, 230,300,800};
  int lowPtBin = 1;
  ...
  getMCR   ( kSample, icent, hmc, ptCut, "");
  getMCR   ( kSample, icent, hdata, ptCut, "trkJetMassRcSubEV");
...
//    jumSun(0,1,50,1);
  ...
  c2->SaveAs(Form("pdfsJMS/JMS_ptCut%d_kSample%d_cent%d_trkVar.pdf",(int)ptCut,kSample,icent));
  ...
  c3->SaveAs(Form("pdfsJMS/trkR_ptCut%d_kSample%d_cent%d_trkVar.pdf",(int)ptCut,kSample,icent));
}
...
void getMCR(int kSample, int icent, TH2D* hmc, int ptCut, TString varName) { 
  ...
  TString jz4;
  if ( kSample == kPbPb ) {
    jz2 = "jetSubstructure_pbpbMC_HION9_jz2_v52Small_april124.root";
    jz3 = "jetSubstructure_pbpbMC_HION9_jz3_v52Small_april124.root";
    jz4 = "jetSubstructure_pbpbMC_HION9_jz4_v52Small_april124.root";
  }
  else if ( kSample == kPP ) {
  ...
  for ( int ijz =2 ; ijz<=4 ; ijz++) {
    ...
    for (Int_t i= 0; i<tr->GetEntries() ; i++) {
      ...
      if ( myJetMc.recoChMassRaw < 0.2) continue; 
