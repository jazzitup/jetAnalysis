...
void getUnfoldingStability(int kSample= kPbPb, int icent = 0, int optX=1, int optY=2) {
  ...
  for ( int ipt = lowPtBin ; ipt<= highPtBin ; ipt++)  {
    ...
    for (int in = 0; in < int(vIter.size()) ; in++)  {
      ...
      handsomeTH1(hmcUnfSq[ipt][in],color[in+2]);
    ...
    if ( ipt == lowPtBin+1 ) {
      TLegend * leg1 = new TLegend(0.1467083,0.2631472,0.6655156,0.4348713,NULL,"brNDC");
      easyLeg(leg1,"Unfolded",0.15);
      for (int in = 0; in < int(vIter.size()) ; in++)  {
  ...
  if (doDATA) {
	...
	handsomeTH1(hdataUnfSq[ipt][in],color[in+2]);
