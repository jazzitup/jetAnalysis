...
void getUnfoldingStability_singlePtBin(int kSample= kPP, int icent = 0, bool matRwt=1, bool specRwt=1, int optX=1, int optY=2, bool applyMDJ=false) {
  ...
  if ( 1==1) { 
    ...
    //  vIter.push_back(12);   color.push_back (32);
    //  vIter.push_back(16);   color.push_back (47);
    //  vIter.push_back(20);     color.push_back (46);
  ...
  */
  ...
  for ( int ipt = lowPtBin ; ipt<= highPtBin ; ipt++)  {
    ...
    for (int in = 0; in < int(vIter.size()) ; in++)  {
      if ( optY==1)  hmcTruth1d[ipt][in]->SetAxisRange(-30,200,"X");
      else if ( optY==2) hmcTruth1d[ipt][in]->SetAxisRange(0.001,0.2999,"X");
      if ( optY==1)    hmcTruth1d[ipt][in]->SetXTitle("m GeV");
      else if ( optY==2)    hmcTruth1d[ipt][in]->SetXTitle("m/p_{T}");
    ...
    drawBin(xBin,ipt,"GeV",0.20,0.78,1,16);
    ...
    for (int in = 0; in < int(vIter.size()) ; in++)  {
      hmcRatio1d[ipt][in] = (TH1D*)hmcUnf1d[ipt][in]->Clone(Form("mcRatio1d_ix%d_in%d",ipt,in));
      hmcRatio1d[ipt][in]->Divide(hmcTruth1d[ipt][in]);
      hmcRatio1d[ipt][in]->SetAxisRange(.5,1.5,"Y");
      hmcRatio1d[ipt][in]->SetAxisRange(0.001,0.299,"X");
      if ( optY==1) hmcRatio1d[ipt][in]->SetAxisRange(0.00,100,"X");
      hmcRatio1d[ipt][in]->SetYTitle("Unfolded/Truth");
      hmcRatio1d[ipt][in]->SetNdivisions(505,"X");
      hmcRatio1d[ipt][in]->SetTitleSize(.12,"X");
      hmcRatio1d[ipt][in]->SetTitleSize(0.1,"Y");
      hmcRatio1d[ipt][in]->SetTitleOffset(.5,"X");
      hmcRatio1d[ipt][in]->SetTitleOffset(.7,"Y");
      
      fixedFontHist(hmcRatio1d[ipt][in],2,2,20);
      if ( in==0)  hmcRatio1d[ipt][in]->Draw();
      else  hmcRatio1d[ipt][in]->Draw("same");
  ...
  //  c1->SaveAs(Form("stabilitiy/mc_coll%d_icent%d_matrixRwt%d_spectraRwt%d_singlePt.pdf",kSample,icent,(int)matRwt, (int)specRwt));
  c1->SaveAs(Form("stabilitiy/mc_coll%d_icent%d_matrixRwt%d_spectraRwt%d_singlePt.pdf",kSample,icent,(int)matRwt, (int)specRwt));
  ...
  if (doDATA) {
    ...
    for ( int ipt = lowPtBin ; ipt<= highPtBin ; ipt++)  {
      ...
      for (int in = 0; in < int(vIter.size()) ; in++)  {
	if ( optY==1) hdataRaw1d[ipt][in]->SetAxisRange(0.00,100,"X");
	else if ( optY==2) hdataRaw1d[ipt][in]->SetAxisRange(0.001,0.2999,"X");
	if ( optY==1)    hdataRaw1d[ipt][in]->SetXTitle("m GeV");
	else if ( optY==2)    hdataRaw1d[ipt][in]->SetXTitle("m/p_{T}");
	...
	hdataRaw1d[ipt][in]->SetNdivisions(505,"X");
        ...
        hdataRaw1d[ipt][in]->SetTitleOffset(.5,"X");
	hdataRaw1d[ipt][in]->SetTitleOffset(.9,"Y");
	
	fixedFontHist(hdataRaw1d[ipt][in],2,2,20);
	if ( in == 0 )  {
	...
	hdataRatio1d[ipt][in]->SetAxisRange(0.001,0.299,"X");
	if ( optY==1)  hdataRatio1d[ipt][in]->SetAxisRange(0.00,100,"X");
	//	hdataRatio1d[ipt][in]->SetYTitle(Form("Ratio to Iter. %d",vIter.at(refIt) ));
	hdataRatio1d[ipt][in]->SetYTitle("Ratio");
	hdataRatio1d[ipt][in]->SetNdivisions(505,"Y");
	hdataRatio1d[ipt][in]->SetNdivisions(505,"X");
	hdataRatio1d[ipt][in]->SetTitleSize(.12,"X");
	hdataRatio1d[ipt][in]->SetTitleSize(0.1,"Y");
	hdataRatio1d[ipt][in]->SetTitleOffset(.5,"X");
	hdataRatio1d[ipt][in]->SetTitleOffset(.6,"Y");
	
	fixedFontHist(hdataRatio1d[ipt][in],2,2,20);
	if ( drawFirst)  { 	hdataRatio1d[ipt][in]->Draw();
	  drawFirst=false;  
	}
	else   hdataRatio1d[ipt][in]->Draw("same");
