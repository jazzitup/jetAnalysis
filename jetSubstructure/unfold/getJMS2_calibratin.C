...
void getJMS2_calibratin(int kSample = kPP, int icent=0, int optX =1, int optY=2, bool doReweight=false) {
  TH1::SetDefaultSumw2();
  int nXbins;
  double xBin[30];
  ...
  cout << "high pt bin = " << highPtBin << endl;
  ...
  */
  ...
  TLegend *leg1 = new TLegend(0.5903614,0.6810526,0.9698795,0.9484211,NULL,"brNDC");
  ...
  for ( int ix = lowPtBin ; ix<=highPtBin ; ix++) { 
    ...
    hs->SetXTitle("(m/p_{T})^2_{Truth}");
    hs->SetYTitle("(m/p_{T})^2_{Reco}/(m/p_{T})^2_{Truth}");
    hs->SetAxisRange(0.001,0.239,"X");
    hs->SetAxisRange(0,12,"Y");
    ...
    for ( int im = 2 ; im <= hs->GetNbinsX() ; im++) {  
      Rx[im-1] = hs->GetBinCenter(im) * sqrt(hs->GetBinContent(im)) ;  //m/pT(2)
      RxErr[im-1] = 0.0001;
      Ry[im-1] = sqrt(hs->GetBinContent(im));
      RyErr[im-1] = hs->GetBinError(im);
      cout << "hs->GetBinCenter(im)  = " << hs->GetBinCenter(im) << endl;
      cout << "hs->GetBinContent(im) = " << hs->GetBinContent(im) << endl;
      cout << Rx[im-1] << endl;
      cout << Ry[im-1] << endl << endl;
  ...
  leg1->Draw();
  c2->SaveAs(Form("jms/jms_kSample%d_icent%d-2-cal.pdf",kSample,icent));
  
  TCanvas* c4 = new TCanvas("c4","",500,500);
  TH1D* htemp = new TH1D("htemp",";(m/p_{T}) * R;R",100,0,0.5);
  htemp->SetAxisRange(0,5.2,"Y");
  htemp->Draw();
  for ( int ix = lowPtBin ; ix<=highPtBin ; ix++) {
    gr[ix]->Draw("same p");
    
    //f1[ix] = new TF1(Form("f1_kSample%d_icent%d_ix%d",kSample,icent,ix),"[0] + [1]*x + [2]*x*x",0.1,0.24);
    //    f1[ix] = new TF1(Form("f1_kSample%d_icent%d_ix%d",kSample,icent,ix),"[0] + [1]*x + [2]*x*x + [3]*x*x*x + [4]*x*x*x*x + [5]*x*x*x*x*x",0.05,0.24);
    //    f1[ix] = new TF1(Form("f1_kSample%d_icent%d_ix%d",kSample,icent,ix),"[0] + [1]/x + [2]/(x*x) ",0.01,0.3);
    ...
    //gr[ix]->Fit("pol4");
    f1[ix] = new TF1(Form("f1_kSample%d_icent%d_ix%d",kSample,icent,ix),"[0] + [1]*x +[2]*x*x + [3]*x*x*x + [4]*x*x*x*x",0.01,0.3);
    f1[ix]->SetParameter(0, 5.95631);
    f1[ix]->SetParameter(1, -100);
    f1[ix]->SetParameter(2, 700);
    f1[ix]->SetParameter(3, -2000);
    f1[ix]->SetParameter(4, 2000);

    //    gr[ix]->Fit(f1[ix],"ll");
    //    gr[ix]->GetFunction(f1[ix]->GetName())->SetLineColor(vColor[ix-lowPtBin]);
    //    if ( doCal )     gr[ix]->GetFunction(f1[ix]->GetName())->SetLineColor(0);
  
  }  
  if ( !doCal)    
    c4->SaveAs(Form("numericalInversion_ksample%d_icent%d-2-cal.pdf",kSample,icent));
  //  if ( !doCal ) {
  ...
  for ( int ix = lowPtBin+1 ; ix<=highPtBin ; ix++) { 
    ...
    hr->SetYTitle("1 #sigma of (m/p_{T})^{2}_{Reco} - (m/p_{T})^{2}_{Truth}");
    hr->SetAxisRange(0.001,0.239,"X");
    hr->SetAxisRange(0,0.04,"Y");
  ...
  leg2->Draw();
  c3->SaveAs(Form("jms/jmr_kSample%d_icent%d-2-cal.pdf",kSample,icent));
  ...
  c2s->SaveAs(Form("jms/jes_kSample%d_icent%d-2-cal.pdf",kSample,icent));
  ...
  leg1s->Draw();
  c3a->SaveAs(Form("jms/jer_kSample%d_icent%d-2-cal.pdf",kSample,icent));
...
void getDist( int kSample, int icent, int optX, int optY, TH2D* hJMS, TH2D* hJMR, TH2D* hJES, TH2D* hJER, bool doReweight ) { 
  ...
  for ( int ix = 0 ; ix <= nXbins+1   ;  ix++) {
    for ( int iy = 0 ; iy <= nYbins+1 ; iy++) {
      ...
      hDist[ix][iy] = new TH1D(Form("hDist_ix%d_iy%d",ix,iy),";(m/p_{T})^{Reco} - (m/p_{T})^{Truth};", 100,0,20);
      hDistJes[ix][iy] = new TH1D(Form("hDistJes_ix%d_iy%d",ix,iy),";p_{T}^{Reco}/p_{T}^{Truth};", 100,0,2);
    }
  }

  TFile* fin = new TFile(Form("fJMScalibration_kSample%d_icent%d_mPt2.root",0,0));
  TF1* fjmscal[30];
  if ( doCal) {  
    for ( int ix = lowPtBin ; ix<=highPtBin ; ix++) { 
  ...
  for ( int ijz =2 ; ijz<=4 ; ijz++) { 
	...
	if ( doCal) {
      ...
      hDist[xx][yy]->Fill(  signFlipM2 /truthM2)  ;
  ...
  for ( int ix = 1 ; ix <= nXbins ; ix++) {
    for ( int iy = 1 ; iy <= nYbins ; iy++)  {
      ...
      hDist[ix][iy]->Fit("gaus", "","",theMean - theRms, theMean + theRms);
