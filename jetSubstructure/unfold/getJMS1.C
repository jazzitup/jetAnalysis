...
void getJMS1(int kSample = kPbPb, int icent=0, int optX =1, int optY=2, bool doReweight=false) {
  TH1::SetDefaultSumw2();
  int nXbins;
  double xBin[30];
  ...
  */
  ...
  for ( int ix = lowPtBin ; ix<=highPtBin ; ix++) { 
    ...
    hs->SetXTitle("(m/p_{T})^1_{Truth}");
    hs->SetYTitle("(m/p_{T})^1_{Reco}-(m/p_{T})^1_{Truth}");
  ...
  leg1->Draw();
  c2->SaveAs(Form("jms/jms_kSample%d_icent%d-1.pdf",kSample,icent));
  ...
  if ( !doCal)    
    c4->SaveAs(Form("numericalInversion_ksample%d_icent%d-1.pdf",kSample,icent));
  //  if ( !doCal ) {
  ...
  //  c2->SaveAs(Form("JMS_vs_truthpT_ksample%d_icent%d_doCal%d-1.pdf",kSample,icent,(int)doCal));
  ...
  leg2->Draw();
  c3->SaveAs(Form("jms/jmr_kSample%d_icent%d-1.pdf",kSample,icent));
  ...
  c2s->SaveAs(Form("jms/jes_kSample%d_icent%d-1.pdf",kSample,icent));
  ...
  leg1s->Draw();
  c3a->SaveAs(Form("jms/jer_kSample%d_icent%d-1.pdf",kSample,icent));
...
void getDist( int kSample, int icent, int optX, int optY, TH2D* hJMS, TH2D* hJMR, TH2D* hJES, TH2D* hJER, bool doReweight ) { 
  ...
  for ( int ix = 0 ; ix <= nXbins+1   ;  ix++) {
    for ( int iy = 0 ; iy <= nYbins+1 ; iy++) {
      ...
      hDist[ix][iy] = new TH1D(Form("hDist_ix%d_iy%d",ix,iy),";(m/p_{T})^{Reco} - (m/p_{T})^{Truth};", 100,-0.2, 0.5);
      hDistJes[ix][iy] = new TH1D(Form("hDistJes_ix%d_iy%d",ix,iy),";p_{T}^{Reco}/p_{T}^{Truth};", 100,0,2);
    }
  }
  ...
  TFile* foutJMS = new TFile(Form("jms/jms_kSample%d_icent%d_doReweight%d-1.root",kSample,icent,doReweight),"recreate");
  for ( int ix = 1 ; ix <= nXbins ; ix++) {
    for ( int iy = 1 ; iy <= nYbins ; iy++)  {
      hDist[ix][iy]->Write();
